create or replace function public.get_product_sales_report_accounting(
  p_start_date text,
  p_end_date text,
...
begin
  ...
  with effective_orders as (
    ...
    where (o.status = 'delivered' or nullif(o.data->>'paidAt','') is not null)
      ...
      ) between v_start_ts and v_end_ts
      and (
        v_zone_text is null
...
revoke all on function public.get_product_sales_report_accounting(text, text, text) from public;
grant execute on function public.get_product_sales_report_accounting(text, text, text) to anon, authenticated;
