-- Fix deduct_stock_on_delivery_v2 signature to match caller in confirm_order_delivery
-- It was called with 3 args (id, items, warehouse_id) but defined with 2.
...
BEGIN
  ...
  -- Clear existing COGS for this order to avoid duplication
  ...
  loop
    ...
    -- Refresh Stock Variables (Trigger updated them)
    if v_item_id_uuid is not null then
