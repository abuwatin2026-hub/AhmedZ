create or replace function public.get_product_sales_report_v5(
...
declare
  v_zone_uuid uuid;
begin
  if nullif(p_zone_id_text, '') is not null
     and p_zone_id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    v_zone_uuid := p_zone_id_text::uuid;
  else
    v_zone_uuid := null;
  end if;
  ...
  with effective_orders as (
    select
      ...
      coalesce(
        case
          when nullif(o.delivery_zone_id::text, '') is not null
    ...
    where (o.status = 'delivered' or nullif(o.data->>'paidAt','') is not null)
      ...
      and (
        v_zone_uuid is null or coalesce(
          case
            when nullif(o.delivery_zone_id::text, '') is not null
                 and o.delivery_zone_id::text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
              then o.delivery_zone_id::text::uuid
