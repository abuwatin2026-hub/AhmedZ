-- Broaden report functions to include all PAID orders, not only delivered

-- Product Sales Report: include any order with paidAt, regardless of status
...
begin
  ...
  with orders_in_range as (
    ...
    where (o.data->>'paidAt') is not null
...
-- Daily sales stats: include all paid orders
...
-- Sales by category: include all paid orders
...
-- Hourly sales stats: include all paid orders
...
-- Payment method stats: include all paid orders
