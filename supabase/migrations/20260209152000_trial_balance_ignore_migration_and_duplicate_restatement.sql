...
begin
  ...
  with eligible_entries as (
    ...
    where (p_start is null or je.entry_date::date >= p_start)
      ...
      and (
        coalesce(je.source_table,'') <> 'base_currency_restatement'
        or je.reference_entry_id is null
        or not exists (
          select 1
          from public.journal_entries je2
          where coalesce(je2.source_table,'') = 'base_currency_restatement'
            and je2.reference_entry_id = je.reference_entry_id
            and je2.created_at > je.created_at
        )
      )
  ),
  filtered_lines as (
    select
      ...
      case
        ...
        when v_old_base is not null
          ...
          and e.entry_date::date < v_lock_date
          and (
            jl.currency_code is null
            or (upper(jl.currency_code) = upper(v_old_base) and (jl.foreign_amount is null or jl.fx_rate is null))
          )
          and coalesce(
            public.get_fx_rate(v_old_base, e.entry_date::date, 'accounting'),
            public.get_fx_rate(v_old_base, e.entry_date::date, 'operational'),
