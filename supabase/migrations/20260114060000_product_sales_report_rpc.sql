...
begin
  ...
  item_sales as (
    select
      ei.item_id,
      -- Quantity Logic: If weight-based (kg/gram), use weight if available.
      -- Note: If unit is 'gram', quantity/weight is in grams.
      sum(
        case
      ...
      -- Sales Logic: (Base Price + Addons) * Quantity
      sum(
        (
  ...
  select
    ...
    -- COGS Logic: Use recorded COGS if available, otherwise fallback to (qty * current_cost)
    coalesce(
      cd.cost_amount,
