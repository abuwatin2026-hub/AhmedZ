...
const rawCodes = String(val('--codes', '')).trim();
const execute = has('--execute');
const reason = String(val('--reason', 'EXPERIMENTAL_ORDERS_FINAL_PURGE')).trim() || 'EXPERIMENTAL_ORDERS_FINAL_PURGE';
...
const defaultCodes = [
  '91025F', '7E748', 'E730BA', '2457A', '74BFA3', '7061E', 'F32DEA', 'EE6DD', '8B7E45', 'DFB2ED',
  '86A00D', 'C5582F', 'E784CA', '6CA3C1', 'EFC675', '53EA87', '682C86', '2E816C', '24D15D', '636E01',
  'EF1A47', 'B8724D', 'BE3C80', '1DF513', '6D8029', 'A2F709', '6D920C', '37478F', 'A47DE9', '322F20',
  'C59C03', 'F1E7F0',
];
const codes = [...new Set((rawCodes ? rawCodes.split(/[,\s]+/) : defaultCodes).map(normalize).filter(Boolean))];
if (!codes.length) throw new Error('No codes provided');
...
const targetSql = `
with input_codes as (
  select distinct upper(regexp_replace(trim(v), '[^A-Za-z0-9]', '', 'g')) as code
...
matched as (
  select
    ...
    c.code,
    case
      when upper(regexp_replace(coalesce(o.invoice_number,''), '[^A-Za-z0-9]', '', 'g')) like '%' || c.code then 'invoice_number'
      when upper(regexp_replace(o.id::text, '[^A-Za-z0-9]', '', 'g')) like '%' || c.code then 'id_suffix'
      when upper(regexp_replace(coalesce(o.data->>'orderCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.orderCode'
      when upper(regexp_replace(coalesce(o.data->>'order_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.order_code'
      when upper(regexp_replace(coalesce(o.data->>'shortCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.shortCode'
      when upper(regexp_replace(coalesce(o.data->>'short_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.short_code'
      when upper(regexp_replace(coalesce(o.data->>'displayCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.displayCode'
      when upper(regexp_replace(coalesce(o.data->>'display_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.display_code'
      when upper(regexp_replace(coalesce(o.data->>'orderNumber',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.orderNumber'
      when upper(regexp_replace(coalesce(o.data->>'order_number',''), '[^A-Za-z0-9]', '', 'g')) = c.code then 'data.order_number'
      else 'mixed'
    end as matched_by
  from public.orders o
  join input_codes c
    on upper(regexp_replace(coalesce(o.invoice_number,''), '[^A-Za-z0-9]', '', 'g')) like '%' || c.code
    or upper(regexp_replace(o.id::text, '[^A-Za-z0-9]', '', 'g')) like '%' || c.code
    or upper(regexp_replace(coalesce(o.data->>'orderCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'order_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'shortCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'short_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'displayCode',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'display_code',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'orderNumber',''), '[^A-Za-z0-9]', '', 'g')) = c.code
    or upper(regexp_replace(coalesce(o.data->>'order_number',''), '[^A-Za-z0-9]', '', 'g')) = c.code
),
target_orders as (
  select
    o.id,
    ...
    o.created_at,
    o.invoice_number,
    array_agg(distinct m.code order by m.code) as matched_codes,
    array_agg(distinct m.matched_by order by m.matched_by) as matched_by
  from public.orders o
  join matched m on m.id = o.id
  group by o.id, o.status, o.created_at, o.invoice_number
)
...
select jsonb_build_object(
  'requestedCodes', (select coalesce(jsonb_agg(code), '[]'::jsonb) from input_codes),
  'matchedCodes', (
    select coalesce(jsonb_agg(x.code), '[]'::jsonb)
    from (
  ...
  'unmatchedCodes', (
    select coalesce(jsonb_agg(i.code), '[]'::jsonb)
    from input_codes i
    where not exists (select 1 from matched m where m.code = i.code)
  ),
  ...
  'sample', (
    ...
    from (
      select o.id::text, o.status, o.created_at, coalesce(o.invoice_number, right(o.id::text, 8)) as inv
...
try {
  const pre = await client.query(auditSql, [codes]);
  const preAudit = pre.rows[0]?.payload ?? {};
  console.log(JSON.stringify({ phase: 'pre_audit', codesCount: codes.length, ...preAudit }, null, 2));

  if (!execute) {
    const targetRows = await client.query(targetSql, [codes]);
    console.log(JSON.stringify({
      phase: 'dry_run_only',
      targets: targetRows.rows.map((r) => ({
        id: r.id,
        status: r.status,
  ...
  await client.query('insert into _target_orders(id) select id from (' + targetSql + ') t', [codes]);
  ...
  await client.query(`create temp table _target_payments on commit drop as
    select p.id
  ...
  await client.query(`create temp table _target_movements on commit drop as
    select im.id
  ...
  await client.query(`create temp table _target_journal_entries on commit drop as
    select je.id
  ...
  await client.query(`create temp table _target_journal_lines on commit drop as
  ...
  const post = await client.query(auditSql, [codes]);
  const postAudit = post.rows[0]?.payload ?? {};
  console.log(JSON.stringify({ phase: 'post_audit', deletedOrders, ...postAudit }, null, 2));
} catch (e) {
  try {
