-- Ensure no overloaded TEXT signature exists for the v3 product report
drop function if exists public.get_product_sales_report_v3(timestamptz, timestamptz, text);
drop function if exists public.get_product_sales_report_v3(timestamp, timestamp, text);

create or replace function public.get_product_sales_report_v3(
