...
begin
  ...
  if not v_reuse_receipt then
    begin
      ...
      values (
        ...
        auth.uid(),
        v_receipt_approval_status,
        v_receipt_req_id,
        v_receipt_requires_approval,
        v_wh,
        v_po.branch_id,
        v_po.company_id,
  ...
  loop
    ...
    select
      coalesce(sum(coalesce(pi.qty_base, pi.quantity, 0)), 0),
    ...
    for v_pi in
      select
        pi.id,
        coalesce(pi.qty_base, pi.quantity, 0) as quantity,
        coalesce(pi.received_quantity, 0) as received_quantity,
        coalesce(pi.unit_cost_base, pi.unit_cost, 0) as unit_cost
      from public.purchase_items pi
      where pi.purchase_order_id = p_order_id
        and pi.item_id = v_item_id
  ...
  for v_item_id, v_ordered, v_received in
    select pi.item_id, coalesce(pi.qty_base, pi.quantity, 0), coalesce(pi.received_quantity, 0)
    from public.purchase_items pi
    where pi.purchase_order_id = p_order_id
  loop
