...
do $$
begin
  drop function if exists public.run_base_currency_historical_restatement(int);
end $$;
...
declare
  ...
  v_fx numeric;
  v_amt_base numeric;
  v_before date := coalesce(p_before_entry_date, current_date);
begin
  if not public.has_admin_permission('accounting.manage') then
    raise exception 'not allowed';
  end if;

  select old_base_currency, new_base_currency
  into v_old_base, v_new_base
...
revoke all on function public.run_base_currency_historical_restatement(int, date) from public;
grant execute on function public.run_base_currency_historical_restatement(int, date) to authenticated;
