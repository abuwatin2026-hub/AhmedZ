...
create policy accounting_periods_owner_write
on public.accounting_periods
for all
...
create or replace function public.is_in_closed_period(p_ts timestamptz)
returns boolean
language sql
stable
security definer
...
$$;
revoke all on function public.is_in_closed_period(timestamptz) from public;
revoke execute on function public.is_in_closed_period(timestamptz) from anon;
grant execute on function public.is_in_closed_period(timestamptz) to authenticated;
...
for each row execute function public.trg_block_journal_in_closed_period();
