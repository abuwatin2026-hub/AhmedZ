-- Migration: Robust Stock Deduction & Recalculation (Fixes INV-001, Silent Failures)
-- Date: 2026-01-31

-- 1. Helper: Recalculate Stock for a Single Item in a Warehouse
...
BEGIN
    -- Try to cast to UUID
    BEGIN
        v_item_id_uuid := p_item_id_text::uuid;
    EXCEPTION WHEN OTHERS THEN
        v_item_id_uuid := NULL;
    END;

    -- 1. Calculate Movements (Available)
    -- In: purchase_in, adjust_in, return_in
    ...
    -- Out: sale_out, adjust_out, return_out, wastage_out
...
GRANT EXECUTE ON FUNCTION public.recalculate_stock_item(text, uuid) TO authenticated;

-- 2. Helper: Recalculate All Stock in Warehouse (For manual fix)
...
GRANT EXECUTE ON FUNCTION public.recalculate_warehouse_stock(uuid) TO authenticated;


-- 3. Updated Deduct Function (Robust & Idempotent)
...
BEGIN
  ...
  loop
    ...
    -- Delete any existing 'sale_out' movements for this item/order to prevent double deduction
    -- This handles the case where a previous run partially succeeded or failed to update stock
    ...
    -- Recalculate stock for this item to ensure we start from a clean state (Physical Count)
    ...
    -- Refresh Stock Variables (Trigger updated them? No, Recalc did, but we need fresh vars after lock)
    -- Actually, we already locked. Recalc updated. We read after recalc. So v_available is correct.
    -- But deleting reservations might have changed 'Reserved' (if recalc included them).
    -- Wait, Recalc included them. Now we deleted them.
    -- So 'Reserved' in DB is now lower.
    -- But 'v_reserved' variable holds the OLD value (read before delete? No, read before delete in this flow?
    -- No, I read 'v_available' BEFORE 'DELETE reservations'?
    -- In my code above:
    -- 1. Recalc (Updates Stock table based on existing reservations).
    -- 2. Select ... INTO v_reserved.
    -- 3. DELETE reservations.
    -- So 'v_reserved' includes the reservations we are about to consume. Correct.
    ...
    -- We deduct the FULL requested amount from Available.
    -- (Reservations were already part of Available, now they are gone from Reserved, so we just decrement Available).
    if v_item_id_uuid is not null then
       UPDATE public.stock_management
       SET available_quantity = available_quantity - v_requested,
           reserved_quantity = reserved_quantity - v_qty_from_res, -- Also decrement reserved part
           last_updated = now()
       WHERE item_id = v_item_id_uuid and warehouse_id = v_warehouse_id;
    else
       UPDATE public.stock_management
...
-- 4. Update Confirm Delivery to Remove Skip Check
...
BEGIN
    ...
    -- REMOVED: The check that skipped deduction if inventory_movements exist.
    -- We now rely on deduct_stock_on_delivery_v2 to be idempotent (cleanup + recalc).
