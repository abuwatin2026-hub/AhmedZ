drop function if exists public.get_product_sales_report_unified(text, text, text, boolean);
...
begin
  ...
  if nullif(trim(coalesce(p_zone_id_text,'')),'') is not null
     and trim(p_zone_id_text) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    v_zone := p_zone_id_text::uuid;
  else
    v_zone := null;
  end if;
  ...
  with effective_orders as (
    select
      ...
      case
        when coalesce(p_invoice_only, false)
    ...
    where (v_zone is null or coalesce(
      o.delivery_zone_id,
      case
        when nullif(o.data->>'deliveryZoneId','') is not null
             and (o.data->>'deliveryZoneId') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  ...
  sales_orders as (
    ...
    where (eo.status = 'delivered' or eo.paid_at is not null)
      and eo.date_by >= v_start_ts
      and eo.date_by <= v_end_ts
  ),
  expanded_items as (
    select
      so.id as order_id,
    ...
    cross join lateral jsonb_array_elements(
      case
        when coalesce(p_invoice_only,false) then
          case
  ...
  returns_base as (
    ...
    where sr.status = 'completed'
      and sr.return_date >= v_start_ts
      and sr.return_date <= v_end_ts
      and (v_zone is null or coalesce(
        o.delivery_zone_id,
        case
          when nullif(o.data->>'deliveryZoneId','') is not null
               and (o.data->>'deliveryZoneId') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  ...
  returns_cost as (
    ...
    where im.reference_table = 'sales_returns'
      ...
      and im.occurred_at >= v_start_ts
      and im.occurred_at <= v_end_ts
      and (
        v_zone is null or exists (
  ...
  period_movements as (
    select
      ...
      as net_qty_period
    from public.inventory_movements im
    where im.occurred_at >= v_start_ts
      and im.occurred_at <= v_end_ts
    group by im.item_id::text
  ),
  item_keys as (
