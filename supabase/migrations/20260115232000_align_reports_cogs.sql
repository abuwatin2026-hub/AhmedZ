...
declare
  ...
  v_total_buying_cost numeric := 0;
  v_total_transport_cost numeric := 0;
  v_total_supply_tax_cost numeric := 0;
  v_total_returns_buying_cost numeric := 0;
  v_total_returns_transport_cost numeric := 0;
  v_total_returns_supply_tax_cost numeric := 0;
...
begin
  ...
  select
    coalesce(sum(oic.quantity * coalesce(mi.buying_price, 0)), 0),
    coalesce(sum(oic.quantity * coalesce(mi.transport_cost, 0)), 0),
    coalesce(sum(oic.quantity * coalesce(mi.supply_tax_cost, 0)), 0)
  into
    v_total_buying_cost,
    v_total_transport_cost,
    v_total_supply_tax_cost
  from public.order_item_cogs oic
  join public.menu_items mi on mi.id::text = oic.item_id::text
  ...
  select
    coalesce(sum(im.quantity * coalesce(mi.buying_price, 0)), 0),
    coalesce(sum(im.quantity * coalesce(mi.transport_cost, 0)), 0),
    coalesce(sum(im.quantity * coalesce(mi.supply_tax_cost, 0)), 0)
  into
    v_total_returns_buying_cost,
    v_total_returns_transport_cost,
    v_total_returns_supply_tax_cost
  from public.inventory_movements im
  ...
  v_total_buying_cost := greatest(v_total_buying_cost - v_total_returns_buying_cost, 0);
  v_total_transport_cost := greatest(v_total_transport_cost - v_total_returns_transport_cost, 0);
  v_total_supply_tax_cost := greatest(v_total_supply_tax_cost - v_total_returns_supply_tax_cost, 0);
  ...
  v_result := json_build_object(
    ...
    'cogs', v_total_cogs,
    'buying_cost', v_total_buying_cost,
    'transport_cost', v_total_transport_cost,
    'supply_tax_cost', v_total_supply_tax_cost,
