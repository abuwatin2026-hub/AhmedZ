...
begin
  ...
  select
    ...
    coalesce(c.payment_terms, 'cash')
  into
    v_commercial_customer_id,
  ...
  if auth.role() <> 'service_role' then
    if not (public.is_staff() or public.is_admin() or v_customer_auth_user_id = auth.uid()) then
      return false;
    end if;
  end if;
...
begin
  ...
  select
    coalesce(
      ...
      nullif(p_updated_data->>'customerId','')::uuid,
      (select c.id from public.customers c where c.auth_user_id = o.customer_auth_user_id limit 1)
    ),
