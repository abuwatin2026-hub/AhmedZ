drop function if exists public.get_product_sales_report_v7(timestamptz, timestamptz, text);

create or replace function public.get_product_sales_report_v7_core(
...
as $$
  select *
  from public.get_product_sales_report_v7_core(
    p_start_date::timestamptz,
    p_end_date::timestamptz,
    p_zone_id_text
  );
...
revoke all on function public.get_product_sales_report_v7_core(timestamptz, timestamptz, text) from public;
grant execute on function public.get_product_sales_report_v7_core(timestamptz, timestamptz, text) to anon, authenticated;
