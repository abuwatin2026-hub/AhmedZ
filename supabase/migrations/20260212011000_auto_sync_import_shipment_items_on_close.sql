...
begin
  ...
  for v_row in
    select
      ...
      coalesce(pr.warehouse_id, new.destination_warehouse_id) as receipt_wh
    from public.purchase_receipts pr
    join public.purchase_receipt_items pri on pri.receipt_id = pr.id
  ...
  loop
    ...
    where b.item_id::text = v_row.item_id_text
      and b.warehouse_id = v_row.receipt_wh;
    ...
    where item_id::text = v_row.item_id_text
      and warehouse_id = v_row.receipt_wh;
  end loop;
