-- Fix: closing import shipment must not update posted inventory_movements (immutable).
-- Instead: update receipt_items + batches + stock avg and post accounting delta entries.
-- This overrides any older trg_close_import_shipment implementation that attempted to UPDATE inventory_movements.
...
begin
  ...
  if abs(coalesce(v_total_delta, 0)) > 1e-6 then
    if abs(coalesce(v_total_delta_sold, 0)) > 1e-6 and not exists (
      ...
      v_clearing := null;
      if v_accounts is not null and nullif(v_accounts->>'import_clearing', '') is not null then
        begin
          v_clearing := (v_accounts->>'import_clearing')::uuid;
        exception when others then
          v_clearing := public.get_account_id_by_code(v_accounts->>'import_clearing');
        end;
      end if;
      v_clearing := coalesce(v_clearing, public.get_account_id_by_code('2210'));
