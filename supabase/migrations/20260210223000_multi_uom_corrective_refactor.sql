...
alter table public.item_uom_units
...
begin
  ...
  select (numerator::numeric / denominator::numeric) into v_factor
  from public.uom_conversions
  where from_uom_id = p_uom_id and to_uom_id = v_base
  limit 1;
...
begin
  ...
  loop
    ...
    select
      coalesce(mi.transport_cost, 0),
      coalesce(mi.supply_tax_cost, 0)
    into
      v_used_transport_cost,
      v_used_supply_tax_cost
    ...
    values (v_pi.item_id::text, v_batch_id, v_wh, coalesce(v_qty_base, 0), null)
    ...
    values (
      ...
      'purchase_orders', p_order_id::text, now(), auth.uid(), jsonb_build_object('purchaseOrderId', p_order_id, 'batchId', v_batch_id, 'warehouseId', v_wh, 'trxQty', coalesce(v_pi.quantity, null), 'trxUomId', case when v_pi.uom_id is null then null else v_pi.uom_id::text end),
      v_batch_id, v_wh
    )
