-- Ensure single, unambiguous signature and aligned filters with order reports
drop function if exists public.get_product_sales_report(timestamptz, timestamptz, text);
drop function if exists public.get_product_sales_report(timestamp, timestamp, text);
drop function if exists public.get_product_sales_report(timestamp, timestamp, uuid);
