...
const cashierMatch = String(val('--cashier-match', 'احمد زنقاح')).trim().toLowerCase();
const execute = has('--execute');
const reason = String(val('--reason', 'CASHIER_OPEN_SHIFT_SALES_PURGE')).trim() || 'CASHIER_OPEN_SHIFT_SALES_PURGE';
...
with candidate_users as (
  select
    ...
    case
      when (
        lower(coalesce(au.full_name,'')) like '%' || $1 || '%'
        or lower(coalesce(au.username,'')) like '%' || $1 || '%'
        or lower(coalesce(au.email,'')) like '%' || $1 || '%'
      ) then 0 else 1
    end as match_rank,
    case
      when exists (
        select 1 from public.cash_shifts cs
        where cs.cashier_id = au.auth_user_id
          and cs.status = 'open'
      ) then 0 else 1
    end as shift_rank
...
cashier_user as (
  ...
  where cu.shift_rank = 0 or cu.match_rank = 0
  order by
    cu.shift_rank,
    cu.match_rank,
    case when cu.role = 'cashier' then 0 else 1 end,
    cu.created_at desc
  limit 1
),
open_shift as (
  select cs.id, cs.cashier_id, cs.opened_at
  from public.cash_shifts cs
  join cashier_user cu on cu.auth_user_id = cs.cashier_id
...
join public.payments p
 ...
 and p.direction = 'in'
join open_shift s on s.id = p.shift_id
...
with candidate_users as (
  select
    au.auth_user_id, au.full_name, au.username, au.email, au.role, au.created_at,
    case
      when (
        lower(coalesce(au.full_name,'')) like '%' || $1 || '%'
        or lower(coalesce(au.username,'')) like '%' || $1 || '%'
...
cashier_user as (
  select cu.auth_user_id, cu.full_name, cu.username, cu.email, cu.role
  from candidate_users cu
  where cu.shift_rank = 0 or cu.match_rank = 0
  order by
    cu.shift_rank,
...
select jsonb_build_object(
  'cashier', (
    select jsonb_build_object(
      'authUserId', cu.auth_user_id::text,
      'fullName', cu.full_name,
      'username', cu.username,
      'email', cu.email,
      'role', cu.role
    )
    from cashier_user cu
  ),
  'openShift', (
    select jsonb_build_object(
      'id', os.id::text,
      'cashierId', os.cashier_id::text,
...
try {
  const pre = await client.query(auditSql, [cashierMatch]);
  const preAudit = pre.rows[0]?.payload ?? {};
  console.log(JSON.stringify({ phase: 'pre_audit', cashierMatch, ...preAudit }, null, 2));
  ...
  await client.query('insert into _target_orders(id) ' + targetSql, [cashierMatch]);
  ...
  const post = await client.query(auditSql, [cashierMatch]);
  const postAudit = post.rows[0]?.payload ?? {};
  console.log(JSON.stringify({ phase: 'post_audit', cashierMatch, deletedOrders, ...postAudit }, null, 2));
} catch (e) {
  try {
