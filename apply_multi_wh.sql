...
begin
  ...
  loop
    ...
    loop
      ...
      values (p_order_id, v_item_id::text, v_item_wh, v_batch.batch_id, v_alloc, now(), now());
    ...
    if not coalesce(v_is_in_store, false) then
      ...
      loop
        ...
        values (
          ...
          jsonb_build_object('orderId', p_order_id, 'warehouseId', v_item_wh, 'batchId', v_batch.batch_id),
          v_batch.batch_id,
          v_item_wh
        )
      ...
      if v_item_batch_text is not null then
        ...
        for update;
        if not found then
          raise exception 'Batch % not found for item % in warehouse %', v_item_batch_text, v_item_id, v_item_wh;
        end if;
