...
begin
  ...
  with orders_in_range as (
    ...
    where o.status = 'delivered'
      ...
      and (
        p_zone_id is null
        or coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) = p_zone_id
      )
      and (
...
begin
  with effective_orders as (
    select
      ...
      coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) as zone_id
    from public.orders o
    where (
      p_zone_id is null
      or coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) = p_zone_id
    ...
    and (
      p_zone_id is null or exists (
        ...
        where o.id = (im.data->>'orderId')::uuid
          and coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) = p_zone_id
      )
    );
