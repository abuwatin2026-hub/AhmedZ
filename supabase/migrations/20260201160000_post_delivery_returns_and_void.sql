...
begin
  ...
  if v_ar_reduction > 0 then
    ...
    set
      original_amount = greatest(0, original_amount - v_ar_reduction),
      open_balance = greatest(0, least(open_balance - v_ar_reduction, greatest(0, original_amount - v_ar_reduction))),
      status = case when greatest(0, least(open_balance - v_ar_reduction, greatest(0, original_amount - v_ar_reduction))) = 0 then 'closed' else status end,
      closed_at = case when greatest(0, least(open_balance - v_ar_reduction, greatest(0, original_amount - v_ar_reduction))) = 0 then now() else closed_at end
    where invoice_id = v_ret.order_id
      and status = 'open';
  end if;
end;
$$;
...
begin
  ...
  if v_ar_amount > 0 then
    update public.ar_open_items
    set
      original_amount = greatest(0, original_amount - v_ar_amount),
      open_balance = greatest(0, least(open_balance - v_ar_amount, greatest(0, original_amount - v_ar_amount))),
      status = case when greatest(0, least(open_balance - v_ar_amount, greatest(0, original_amount - v_ar_amount))) = 0 then 'closed' else status end,
      closed_at = case when greatest(0, least(open_balance - v_ar_amount, greatest(0, original_amount - v_ar_amount))) = 0 then now() else closed_at end
