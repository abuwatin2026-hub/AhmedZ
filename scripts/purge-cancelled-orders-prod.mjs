...
const reason = String(val('--reason', 'PURGE_CANCELLED_ORDERS')).trim() || 'PURGE_CANCELLED_ORDERS';
...
where
  ...
  or coalesce(o.data->>'voidedAt', '') <> ''
  or lower(coalesce(o.data->>'voided', 'false')) = 'true'
  or lower(coalesce(o.data->>'isVoided', 'false')) = 'true'
`;
...
select jsonb_build_object(
  ...
  'statusBreakdown', (
    ...
    from (
      select lower(coalesce(o.status,'unknown')) as status, count(*)::int cnt
      from public.orders o
      join target_orders t on t.id = o.id
      group by 1
    ) s
  ),
...
try {
  const pre = await client.query(auditSql);
  const preAudit = pre.rows[0]?.payload ?? {};
  console.log(JSON.stringify({ phase: 'pre_audit', ...preAudit }, null, 2));
  ...
  await client.query('insert into _target_orders(id) ' + targetSql);
  ...
  const post = await client.query(auditSql);
