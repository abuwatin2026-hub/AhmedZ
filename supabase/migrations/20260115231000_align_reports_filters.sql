...
begin
  ...
  computed as (
    select
      ...
      sum(
        case
          when li.unit_type in ('kg', 'gram') and li.weight > 0
            then (li.weight * greatest(li.quantity, 1))
          else greatest(li.quantity, 0)
        end
      ) as qty_sold,
      sum(
        (
          (
            case
              when li.unit_type = 'gram'
                   and li.price_per_unit > 0
                   and li.weight > 0 then (li.price_per_unit / 1000.0) * li.weight
              when li.unit_type in ('kg', 'gram')
                   and li.weight > 0 then li.price * li.weight
              else li.price
            end
            +
            coalesce((
              ...
              from jsonb_each(li.addons) as a(key, addon_value)
            ), 0)
          )
          *
          case
            when li.unit_type in ('kg', 'gram') and li.weight > 0
              then greatest(li.quantity, 1)
            else greatest(li.quantity, 0)
          end
        )
      ) as sales_amount
    from line_items li
    group by li.cat_name
  )
  select
    c.cat_name as category_name,
    coalesce(c.sales_amount, 0) as total_sales,
    coalesce(c.qty_sold, 0) as quantity_sold
  from computed c
  order by 2 desc;
end;
$$;
