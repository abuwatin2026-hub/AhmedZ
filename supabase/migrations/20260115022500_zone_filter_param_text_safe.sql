...
begin
  ...
  with orders_in_range as (
    ...
    where o.status = 'delivered'
      ...
      and (
        v_zone_uuid is null
        or coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) = v_zone_uuid
      )
      and (
    ...
    and (
      v_zone_uuid is null or exists (
        ...
        where o.id = (im.data->>'orderId')::uuid
          and coalesce(o.delivery_zone_id, nullif(o.data->>'deliveryZoneId','')::uuid) = v_zone_uuid
      )
    );

  v_total_cogs := greatest(v_total_cogs - v_total_returns_cogs, 0);

  if v_zone_uuid is null then
