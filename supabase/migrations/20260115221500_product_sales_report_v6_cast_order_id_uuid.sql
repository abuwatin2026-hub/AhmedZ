create or replace function public.get_product_sales_report_v6(
...
begin
  ...
  with effective_orders as (
    select
      case
        when o.id::text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then o.id::text::uuid
        else null
      end as id_uuid,
      o.status,
    ...
    where (o.status = 'delivered' or nullif(o.data->>'paidAt','') is not null)
      ...
      and (
        v_zone_text is null or coalesce(nullif(o.delivery_zone_id::text, ''), nullif(o.data->>'deliveryZoneId','')) = v_zone_text
      )
  ),
  expanded_items as (
    select
      eo.id_uuid as order_id,
  ...
  cogs_data as (
    ...
    where oic.order_id in (select id_uuid from effective_orders where id_uuid is not null)
