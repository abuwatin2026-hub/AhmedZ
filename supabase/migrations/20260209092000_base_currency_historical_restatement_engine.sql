...
do $$
begin
  if to_regclass('public.base_currency_migration_state') is null then
    create table public.base_currency_migration_state (
      id text primary key,
...
do $$
begin
  if not exists (select 1 from public.base_currency_migration_state where id = 'sar_base_lock') then
    insert into public.base_currency_migration_state(id, old_base_currency, new_base_currency, locked_at)
    values ('sar_base_lock', 'YER', 'SAR', now());
  end if;
end $$;

do $$
begin
  if to_regclass('public.base_currency_migration_entry_map') is null then
    create table public.base_currency_migration_entry_map (
    ...
    create index if not exists idx_base_currency_migration_entry_map_status on public.base_currency_migration_entry_map(status);
  end if;
end $$;
