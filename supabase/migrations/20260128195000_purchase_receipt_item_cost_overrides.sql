...
alter table public.purchase_receipt_items
add column if not exists supply_tax_cost numeric not null default 0;
...
begin
  ...
  loop
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'warehouseId', v_wh,
        'transport_unit', v_used_transport_cost,
        'transport_total', v_used_transport_cost * v_qty,
        'supplier_tax_unit', v_used_supply_tax_cost,
        'supplier_tax_total', v_used_supply_tax_cost * v_qty
      ),
      v_batch_id,
      v_wh
    )
  ...
  order by ar.created_at desc
  limit 1;
  if v_required_po and v_po_req_id is null and v_all_received and public.is_owner() then
    insert into public.approval_requests(
      target_table, target_id, request_type, status,
      requested_by, approved_by, approved_at,
      payload_hash
