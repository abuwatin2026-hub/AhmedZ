...
-- Harden reserve contract for in-store multi-warehouse lines:
-- 1) enforce line warehouseId for in-store orders
-- 2) validate/normalize UOM factor against item_uom_units when uomCode is provided
...
begin
  ...
  if p_order_id is not null then
    select coalesce(o.data->>'orderSource', '')
    into v_order_source
    from public.orders o
    where o.id = p_order_id;
    v_require_line_warehouse := (v_order_source = 'in_store');
  end if;
