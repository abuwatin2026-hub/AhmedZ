...
begin
  ...
  item_sales as (
    select
      ...
      sum(
        case
          when coalesce(nullif(mi.unit_type, ''), nullif(ei.unit_type, ''), 'piece') in ('kg', 'gram')
               and ei.weight > 0 then ei.weight
          else ei.quantity
        end
      ) as qty_sold,
      sum(
        (
          case
            when coalesce(nullif(mi.unit_type, ''), nullif(ei.unit_type, ''), 'piece') = 'gram'
                 and ei.price_per_unit > 0 then ei.price_per_unit / 1000.0
            else ei.price
          end
          +
          coalesce((
  ...
  left join item_sales its on its.item_id_text = mi.id::text
  left join returns_movements rm on rm.item_id_text = mi.id::text
  left join cogs_data cd on cd.item_id_text = mi.id::text
  left join public.stock_management sm on sm.item_id::text = mi.id::text
  left join period_movements pm on pm.item_id_text = mi.id::text
