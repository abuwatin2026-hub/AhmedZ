...
begin
  ...
  if v_type in ('fx_revaluation','smoke_fx_revaluation') then
  ...
  elsif v_type in ('auto_settlement','smoke_auto_settlement') then
  ...
  elsif v_type in ('ledger_snapshot','smoke_ledger_snapshot') then
  ...
  elsif v_type in ('open_items_snapshot','smoke_open_items_snapshot') then
    v_period_end := nullif(v_job.payload->>'asOf','')::date;
    if v_period_end is null then
      raise exception 'asOf required';
    end if;
