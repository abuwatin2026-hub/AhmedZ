create or replace function public.get_product_sales_report_v8(
...
begin
  ...
  returns_sales as (
    select
      rigv.item_id_text,
      sum(rigv.qty_returned) as returned_quantity,
      sum(
        case
  ...
  returns_cost as (
    select
      ...
      sum(im.quantity) as returned_quantity_cost,
  ...
  select
    ...
    coalesce(sl.qty_sold, 0) as quantity_sold,
    coalesce(sl.gross_sales, 0) as gross_sales,
    coalesce(rs.returned_quantity, 0) as returned_quantity,
    coalesce(rs.returned_sales, 0) as returned_sales,
    greatest(coalesce(sl.gross_sales, 0) - coalesce(rs.returned_sales, 0), 0) as net_sales,
    coalesce(cg.gross_cost, 0) as gross_cost,
    coalesce(rc.returned_cost, 0) as returned_cost,
    greatest(coalesce(cg.gross_cost, 0) - coalesce(rc.returned_cost, 0), 0) as net_cost,
    (
...
revoke all on function public.get_product_sales_report_v8(timestamptz, timestamptz, uuid, boolean) from public;
grant execute on function public.get_product_sales_report_v8(timestamptz, timestamptz, uuid, boolean) to anon, authenticated;
